...
class Factory:
    ...
    def info_proizvodstvo_za_sekunda(self, world):
        ...
        return kolko_imam * (2 ** (self.stepen - 1)) * bonus * world["tickspeed_multiplier"]
...
def update_ui():
    ...
    btn_monster.config(text=f"🍍 Купи Monster Energy ({world['monster_count']}) - Цена: {format_number(world['monster_cena'])}")
    ...
    label_tickspeed.config(text=f"Текущ Tickspeed: {world['tickspeed_multiplier']:.3f}x")
...
поредност_бутони = ["iliq", "pushka", "megailiq", "gigailiq", "terailiq", "petailiq", "exailiq", "zettailiq"]

ИМЕНА_ЗА_UI = {
    "iliq": "Разглобена Клавиатура", "pushka": "Загуба", "megailiq": "Изгубено 1v1", "gigailiq": "Wallhack",
    "terailiq": "Лоши Хора", "petailiq": "Лоши Времена", "exailiq": "Еврейн", "zettailiq": "Синагога"
}

factories = {
    "zettailiq": Factory("zettailiq", basecena=1e24, cost_multiplier=1e15, pravi="exailiq"),
    "exailiq": Factory("exailiq", basecena=1e18, cost_multiplier=1e12, pravi="petailiq"),
    "petailiq": Factory("petailiq", basecena=1e13, cost_multiplier=1e10, pravi="terailiq"),
    "terailiq": Factory("terailiq", basecena=1e9, cost_multiplier=1e8, pravi="gigailiq"),
    "gigailiq": Factory("gigailiq", basecena=1e6, cost_multiplier=1e6, pravi="megailiq"),
    "megailiq": Factory("megailiq", basecena=10000, cost_multiplier=1e5, pravi="pushka"),
...
world = {
    "rage": 10, "iliq": 0, "pushka": 0, "megailiq": 0, "gigailiq": 0,
    "terailiq": 0, "petailiq": 0, "exailiq": 0, "zettailiq": 0,
    "tickspeed_multiplier": 1.0, "monster_count": 0, "monster_cena": 1000
}
...
label_tickspeed = tk.Label(root, text="", font=("Courier", 10, "italic"), bg=DARK_BG, fg="#AAAAAA")
label_tickspeed.pack(pady=(0, 15))

label_rage = tk.Label(root, text="", font=("Courier", 22, "bold"), bg=DARK_BG, fg="#FF4D4D")
...
label_instructions = tk.Label(root, text=инструкции_текст, font=("Helvetica", 9, "italic"), bg=DARK_BG, fg="#BBBBBB", justify="center")
label_instructions.pack(anchor="center", pady=(5, 0))

divider = tk.Label(root, text="-----------------------------------------------------------------------------", font=("Courier", 10), bg=DARK_BG, fg="#555555")
divider.pack()
...
for i, b_name in enumerate(поредност_бутони):
    row_label = tk.Label(game_frame, text="", font=("Courier", 10), anchor="w", justify="left", bg=DARK_BG, fg="#FFFFFF")
    ...
    row_btn.bind("<ButtonPress-1>", lambda event, name=b_name: start_buying_building(name))
